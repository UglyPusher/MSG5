...
#include "bootstrap/spec/ApplyMetaDataSpec.h"
...
namespace msg5::bootstrap::commands {
    int RunApplyMetaData(int argc, char** argv) {
        auto ro = sources::ResolveFor(argc, const_cast<const char* const*>(argv),
            spec::MakeSpec_ApplyMetaData());
        const std::string app_dsn = ro.values.count("app_dsn") ? ro.values.at("app_dsn") : "";
        const std::string data_dir = ro.values.count("data_dir") ? ro.values.at("data_dir") : "";
        ...
        if (app_dsn.empty() || data_dir.empty()) {
            std::cerr << "[apply-meta-data] required params missing.\n";
            bootstrap::ui::print_usage();
            return 2;
        }
        
        if (!(tx_mode == "per-file" || tx_mode == "single" || tx_mode == "none")) {
            std::cerr << "[apply-meta-data] invalid --tx-mode: " << tx_mode
                << " (allowed: per-file|single|none)\n";
            return 2;
        }

        std::vector<std::filesystem::path> files;
        try { files = msg5::utils::list_files_with_extension(std::filesystem::path(data_dir), ".sql"); }
        catch (const std::exception& ex) { std::cerr << "[apply-meta-data] FS error: " << ex.what() << "\n"; return 2; }
        if (files.empty()) { std::cout << "[apply-meta-data] No .sql files in: " << data_dir << "\n"; return 0; }
        ...
        if (!force) { std::cout << "[apply-meta-data] not forced; nothing executed.\n"; return 0; }
        if (!yes && !bootstrap::ui::msg5_prompt_yes_no("Proceed applying meta data?")) { std::cout << "[apply-meta-data] canceled.\n"; return 0; }

        PgExecutor db{ app_dsn };
        auto run_file = [&](const std::filesystem::path& p)->bool {
            std::string sql;
            if (!read_file_text(p, sql)) {
                std::cerr << "[apply-meta-data] cannot open: " << p << "\n";
                return false;
            }
        ...
        if (tx_mode == "single") {
            ...
            for (auto& f : files) {
                try { db.exec("begin;"); bool ok = run_file(f); db.exec(ok ? "commit;" : "rollback;"); if (!ok) { any_err = true; if (!cont_on_err) break; } }
                    ...
                    any_err = true; std::cerr << "[apply-meta-data] TX error on file " << f.filename().string() << ": " << ex.what() << "\n"; if (!cont_on_err) break;
                }
            }
