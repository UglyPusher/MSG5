...
// -----------------------------------------------------------------------------
// P0.2 Ч Safe SQL quoting helpers
...
//  * DO NOT use ident quoting for connection strings (DSN) Ч only for SQL text.
// -----------------------------------------------------------------------------
...
// Optional args: owner, template, encoding Ч pass empty string to skip.
...
Notes:
  ...
  - Default mode is DRY-RUN and prints a plan ([plan]). To execute, add --force (or auto-confirm with --yes).
    ...
    // чтение JSON из stdin, если флаг включЄн
    ...
    void merge_validate(const json& j, Inputs& in) {
        ...
        get("dsn", in.dsn);
        // совместимость с create-database: можно переиспользовать пол€
        get("host", in.host);
        get("port", in.port);
        get("user", in.owner);      // не об€зательно
    ...
    int cmd_create_database(Inputs in) {
        ...
        // проста€ интерактивна€ докомплектаци€
    ...
    int cmd_apply_meta(Inputs in) {
        ...
        // интерактивно доберЄм недостающее
        if (in.app_dsn.empty()) {
        ...
        try {
            ...
            // идемпотентность: если мета уже есть Ч ничего не делаем
            if (msg5::dbprobe::meta_schema_present(app)) {
                std::cout << "[apply] meta.meta_schema already present Ч nothing to do\n";
                return 0;
            }
